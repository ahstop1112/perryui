...
export interface DivListSubRowProps {
  subRow: RowData
  columns: ColumnMap
}
...
const DivListSubRow = ({ subRow, columns }: DivListSubRowProps) => {
  return (
    <TableRow className={styles.subRow}>
      {Object.keys(subRow)
        .filter(col => Object.keys(columns).includes(col))
        .map(col => (
          <TableCell key={col} className={`${styles.subRowCell} ${columns[col]?.className ?? ''}`}>
