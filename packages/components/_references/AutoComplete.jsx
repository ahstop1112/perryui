...
const AutoComplete = ({ inputField, ...props }) => {
  ...
  const convertI18nArr = (data) => {
    ...
    data
      ...
      .map((item) => {
        if (item?.num) {
          newData.push({ label: t(item.label), value: item.value, num: item.num });
        } else if (item?.key) {
          newData.push({ label: t(item.label), value: item.value, key: item.key });
        } else {
  ...
  const convertI18nValue = (dropdownVal) => {
    return {
      label: dropdownVal.label && dataFromApi ? dropdownVal.label : t(dropdownVal.label),
      value: dropdownVal.value,
    };
  };

  const returnPreviewValue = (value) => {
    let tmpValue = ``;
    if (typeof value !== "string") {
      ...
      tmpValue = !dataFromApi ? (name.includes("ccy") ? t(`bpmp:bpmp.currency.${value}`) : `${t(`${value}`)}`) : value;
    } else if (!value) {
      tmpValue = "----";
    }
    return tmpValue;
  ...
  // value = [Object]
  // value = undefined
  // 1. length === 1, Object, e.g. { label: '', value : ''}
  // 2. length > 2, Array, e.g. [{label: 'abc', value: 'abc'}, {label: 'def', 'def'}]
  // 3. value === 'text', e.g. value?.value

  return (
    <>
      {isPreview && isShown ? (
        <div className={classes.preview}>{returnPreviewValue(value)}</div>
      ) : (
        <div
        ...
        >
          <Select
            ...
            options={data && data.length > 0 ? (dataFromApi ? data : convertI18nArr(data)) : []}
            textFieldProps={{
              InputLabelProps: {
                shrink: true,
              },
      ...
      {setOthersMsg && !isPreview && (
        <ErrorMsg className={classes.errorMsg} type="error" label={t("flows:flows.WOC.othersErrorMsg")} />
      )}
    </>
  );
};
